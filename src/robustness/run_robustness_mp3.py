...
RESULTS_PATH = DATA_DIR / "results_robustness_mp3.csv"
...
def build_mp3_dev_mfcc(bitrate: str) -> Path:
    safe_bitrate = bitrate.replace("k", "kbps")
    out_path = DATA_DIR / f"robust_dev_mfcc_mp3_{safe_bitrate}.csv"
...
def build_mp3_dev_tda(bitrate: str) -> Path:
    safe_bitrate = bitrate.replace("k", "kbps")
    out_path = DATA_DIR / f"robust_dev_tda_mp3_{safe_bitrate}.csv"
...
def main() -> None:
    ...
    for bitrate in bitrates:
        print(f"Preparing degraded dev for MP3 compression at {bitrate}")

        mfcc_dev_csv = build_mp3_dev_mfcc(bitrate=bitrate)
        tda_dev_csv = build_mp3_dev_tda(bitrate=bitrate)

        for seed in seeds:
            print(f"Running sample_size={sample_size}, seed={seed}, mp3={bitrate}")
