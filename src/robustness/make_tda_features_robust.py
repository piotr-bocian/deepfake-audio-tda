...
from src.clean.features_tda import extract_tda_features_from_mfcc
from src.robustness.audio_augment import apply_degradation


def build_tda_feature_dataframe_robust(
    audio_dir: str | Path,
    ...
    for i, (_, meta) in enumerate(
        tqdm(labels_df.iterrows(), total=len(labels_df), desc=f"TDA robust {audio_dir.name}")
    ):
